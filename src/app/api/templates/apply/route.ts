...
export async function POST(request: Request) {
  ...
  const { boardId, templateId, isCustom } = await request.json();
  ...
  if (isCustom) {
    const customTemplate = await prisma.customTemplate.findFirst({
      where: { id: templateId, userId: session.id },
    });
    if (!customTemplate) {
      return NextResponse.json({ error: "Template not found" }, { status: 404 });
    }
    templateCards = customTemplate.cards as any[];
  } else {
